...
export class PercentageVolumeOscillatorIndicatorStrategy extends IndicatorCalculationStrategy {
	static $t: Type = markType(PercentageVolumeOscillatorIndicatorStrategy, 'PercentageVolumeOscillatorIndicatorStrategy', (<any>IndicatorCalculationStrategy).$type);
	...
	calculateIndicator(dataSource: FinancialCalculationDataSource, supportingCalculations: FinancialCalculationSupportingCalculations): boolean {
		...
		while (shortEma.moveNext() && longEma.moveNext()) {
			let pvo: number = supportingCalculations.makeSafe(100 * (shortEma.current - longEma.current) / longEma.current);
