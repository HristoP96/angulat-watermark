...
export class SlowStochasticOscillatorIndicatorStrategy extends IndicatorCalculationStrategy {
	static $t: Type = markType(SlowStochasticOscillatorIndicatorStrategy, 'SlowStochasticOscillatorIndicatorStrategy', (<any>IndicatorCalculationStrategy).$type);
	...
	calculateIndicator(dataSource: FinancialCalculationDataSource, supportingCalculations: FinancialCalculationSupportingCalculations): boolean {
		...
		let slowPercentK: IEnumerable$1<number> = supportingCalculations.eMA.strategy(percentK, 3);
		let i: number = 0;
		for (let value of fromEnum<number>(slowPercentK)) {
