...
export class PolyBezierSegmentData extends SegmentData {
	static $t: Type = markType(PolyBezierSegmentData, 'PolyBezierSegmentData', (<any>SegmentData).$type);
	constructor(initNumber: number);
	constructor(initNumber: number, poly: PolyBezierSegment);
	...
	constructor(initNumber: number, ..._rest: any[]) {
		...
		switch (initNumber) {
			...
			case 1:
			{
				let poly: PolyBezierSegment = <PolyBezierSegment>_rest[0];
				this.points = new List$1<PointData>((<any>PointData).$type, 0);
				for (let i = 0; i < poly.points.count; i++) {
					this.points.add(PointData.fromPoint(poly.points._inner[i]));
				}
			}
			break;
