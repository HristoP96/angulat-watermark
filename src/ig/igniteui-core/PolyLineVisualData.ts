...
export class PolyLineVisualData extends PrimitiveVisualData {
	static $t: Type = markType(PolyLineVisualData, 'PolyLineVisualData', (<any>PrimitiveVisualData).$type);
	...
	constructor(initNumber: number, name: string, line: Polyline);
	...
	constructor(initNumber: number, ..._rest: any[]) {
		...
		switch (initNumber) {
			...
			case 1:
			{
				...
				let line: Polyline = <Polyline>_rest[1];
				super(1, name);
				this._points = null;
				this.points = new List$1<PointData>((<any>PointData).$type, 0);
				for (let i = 0; i < line.points.count; i++) {
					this.points.add(PointData.fromPoint(line.points._inner[i]));
				}
